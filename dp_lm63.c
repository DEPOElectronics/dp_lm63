			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	//Disable external term
	i2c_smbus_write_byte_data(client, LM96163_REG_TRUTHERM, 0x0);
...
static const struct of_device_id __maybe_unused lm63_of_match[] = {
	{
		.compatible = "national,dp_lm63",
		.data = (void *)lm63
	},
	{
		.compatible = "national,dp_lm64",
		.data = (void *)lm64
	},
	{
...
MODULE_DESCRIPTION("DEPO LM63 driver");
MODULE_LICENSE("GPL");
